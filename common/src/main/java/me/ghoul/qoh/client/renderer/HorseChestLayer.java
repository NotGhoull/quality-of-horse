...
public class HorseChestLayer<T extends LivingEntity, M extends EntityModel<T>> extends RenderLayer<T, M> {
    ...
    public void render(PoseStack poseStack, MultiBufferSource multiBufferSource, int i, T entity, float f, float g, float h, float j, float k, float l) {
        if (!((IHorseWithChest) entity).qoh$hasChest()) {
            return;
        }


        chestModel.render(poseStack, multiBufferSource.getBuffer(RenderType.entityCutoutNoCull(CHEST)), i, LivingEntityRenderer.getOverlayCoords(entity, 0.f));
    }
}
